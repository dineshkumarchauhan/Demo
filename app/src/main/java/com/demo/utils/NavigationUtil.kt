 ...
 * */
fun NavController.navigateId(destinationId: Int, bundle: Bundle? = null) {
    try {
        navigate(
            destinationId,
            bundle,
            NavOptions.Builder()
                .setEnterAnim(ANIM_ALIAS.slide_in_right)
                .setExitAnim(ANIM_ALIAS.slide_out_left)
                .setPopEnterAnim(ANIM_ALIAS.slide_in_left)
                .setPopExitAnim(ANIM_ALIAS.slide_out_right)
                .build()
        )
    } catch (e: Exception) {
        e.printStackTrace()
